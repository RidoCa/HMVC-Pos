...
class Transaction_db extends CI_Model
{
    ...
    public function storeID()
    {
        ...
        $this->db->from('store');
        $this->db->join('user', 'user.store_id = store.store_id', 'inner');
    ...
    public function lastID()
    {
        ...
        $this->db->like('transaction_id', 'INV' . $storeID['store_id'] . dateAuto(date('Ymd')), 'both');
    ...
    public function transaction_id()
    {
        ...
        if (!empty($lastID)) {
            $this->db->select('right(transaction_id,5) as kode', false);
            $this->db->order_by('transaction_id', 'desc');
            $this->db->limit(1);
            $query = $this->db->get('transaction');
            if ($query->num_rows() <> 0) {
            ...
            $result_id = 'INV' . $storeID['store_id'] . dateAuto(date('Ymd')) . $kodemax;
        } else {
    ...
    {
        $this->db->select('SUM(transaction_qty) as qty, product_name, SUM(transaction_price) as price, transaction_note, product.product_id');

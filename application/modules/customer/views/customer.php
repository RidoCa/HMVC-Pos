 <div class="container my-4">
     <h6 class="page-subtitle">List Customer
        ...
        ?>
             <div class="card shadow-sm border-0 mb-3">
                 <div class="card-body">
                     <div class="row">
                         <div class="col">
                             <h5 class="font-weight-normal mb-1"><?= $row['customer_name'] ?></h5>
                             <p class="text-mute small text-secondary mb-2"><?= $row['division_name'] ?></p>
                         </div>
                         <div class="col-auto align-self-center">
                             <a class="btn btn-35 default-shadow border-0 bg-danger btnDelete" title="delete" data-code="<?= $row['customer_id'] ?>">
                                 <i class="far fa-trash-alt"></i>
                             </a>
                             <a class="btn btn-35 default-shadow border-0 bg-warning btnEdit" title="edit" data-code="<?= $row['customer_id'] ?>">
             ...
             <div class="modal-content">
                 <form id="form">
                     ...
                     <div class="modal-body">
                         ...
                         <div class="form-group">
                             <label class="text-mute small">Phone</label>
                             <input type="number" class="form-control rounded-right-custom rounded-left-custom" id="phone" name="phone" maxlength="13">
                             <span class="help-block small text-danger"></span>
                         </div>
                         <div class="form-group">
                             <label class="text-mute small">Address</label>
                             <textarea type="text" class="form-control rounded-right-custom rounded-left-custom" id="address" name="address"></textarea>
                             <span class="help-block small text-danger"></span>
                         </div>
                         <div class="form-group">
 ...
 <script type="text/javascript">
     ...
     $(document).ready(function() {
         $('.btnEdit').click(function() {
             ...
             $.ajax({
                 url: "<?= site_url('customer/ajax_edit/') ?>" + id,
                 type: "GET",
                 dataType: "JSON",
                 success: function(data) {
                     ...
                     $('[name="phone"]').val(data.customer_phone);
                     $('[name="address"]').val(data.customer_address);
                     if (data.is_active == 1) {
     ...
     function save() {
         ...
         if (save_method == 'add') {
             url = "<?= site_url('customer/ajax_add') ?>";
         } else if (save_method == 'delete') {
             url = "<?= site_url('customer/ajax_delete') ?>";
         } else {
             url = "<?= site_url('customer/ajax_update') ?>";
         }
